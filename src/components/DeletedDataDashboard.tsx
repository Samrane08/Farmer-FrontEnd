...
const DeletedDataDashboard: React.FC = () => {
  ...
  const getDeletedBankData = async () => {
    ...
    try {
      ...
      const response = await apiCall<any>(`${getDeletedBankDataResponse}?${query}`, bearerToken, "GET");
  ...
  const downloadFiles = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await apiCall<Blob>(DownloadDeletedDataDashboard, bearerToken, "GET", undefined, "blob");
